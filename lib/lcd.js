...
var Controllers = {
  JHD1313M1: {
    ...
    initialize: {
      value: function(opts) {
        ...
        this.queue.schedule(function (done) {
    ...
    command: {
      value: function(mode, value, waitMs) {
        ...
        this.queue.schedule(function (done) {
          // Create a done callback bound to waitMs
          done = done.bind(null, waitMs);
          ...
          this.writeBits(this.OP.COMMAND, value);
  ...
  PCF8574: {
    ...
    resetPulse: function () {
        // This is one of typically several pulses to fully reset LCD
        this.queue.schedule(function (done) {
          //
...
function LCD(opts) {
  ...
  // Synchronizes commands with waits.  Uses callbacks for discrete functions between waits
  this.queue = queue('five.LCD', 100);
...
LCD.prototype.command = function(mode, value, waitMs) {
  this.queue.schedule(function (done) {
    // Create a done callback bound to waitMs
    done = done.bind(null, waitMs);
    ...
    if (this.bitMode === 4) {  
      this.send(value >> 4, function () {
        this.send(value, done);
      }.bind(this));
    } else {
      this.send(value, done);
    }
...
LCD.prototype.send = function(value, done) {
  var pin = 0;
  var mask = {
    4: 8,
    8: 128
  ...
  [{
    ...
    this.queue.schedule(function (done) {
      this.io.digitalWrite(this.pins.en, this.io[val.state]);
      done(val.wait);
      // This allows command queue to go to next command
      if (val.done) {
        val.done();
      }
    }.bind(this));
...
LCD.prototype.clear = function() {
  this.command(this.OP.CLEAR, null, 2);
  return this;
};

LCD.prototype.home = function() {
  this.command(this.OP.HOME, null, 2);
  return this;
};
